...
public class RideController {
    ...
    @PostMapping("/")
    public ResponseEntity<RideResponse> createRide(@AuthenticationPrincipal CustomUserDetails currentUser, @Valid @RequestBody RideDto rideDto){
        return ResponseEntity.ok(new RideResponse(rideService.createRide(currentUser, rideDto)));
    }

    @PutMapping("/{id}")
    public ResponseEntity<RideResponse> updateRide(@AuthenticationPrincipal CustomUserDetails currentUser, @PathVariable Long id, @Valid @RequestBody RideDto rideDto){
        return ResponseEntity.ok(new RideResponse(rideService.updateRide(currentUser, id, rideDto)));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteRide(@AuthenticationPrincipal CustomUserDetails currentUser, @PathVariable Long id){
        rideService.deleteRide(currentUser, id);
        return ResponseEntity.noContent().build();
    }

    @PostMapping("/{id}/book")
    public ResponseEntity<RideResponse> bookRide(@AuthenticationPrincipal CustomUserDetails currentUser, @PathVariable Long id){
